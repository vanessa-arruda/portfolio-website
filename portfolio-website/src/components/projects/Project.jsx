...
export const Project = () => {
    ...
    return (
        <section>
            <h1 className="project-section-title" id='projects-title'>Featured Projects</h1>
            {/* <div>{projectsList}</div> */}
            {projectsList.map((project) => (
                <div key={project.id} className="project-main-container">
                    <div className='card-container'>
                        <div className="card-right-container card-front">
                            <img className="project-img" src={project.img} alt="project screenshot"/>
                            <div className="project-btns">
                                <a target="_blank" rel='noreferrer' href={project.demo_link} className="demo-btn">Live Demo</a>
                                <a target="_blank" rel='noreferrer' href={project.repository_link} className="repo-btn">View code</a>
                            </div>
                        </div>
                        <div className="card-left-container card-back">
                            <h3>{project.name}</h3>
                            <p>{project.description}</p>
                            <div className="card-skill-container">
                                {project.skills.map((skill) => (
                                    <span key={`${project.id}_${skill}`} className="card-skill">{skill}</span>
                                ))}
                            </div>
                        </div>
                    </div>
